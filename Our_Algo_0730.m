function [alpha,bias,Ktr,K_Delta,E] = Our_Algo_0730(X_miss,maskInv,y,pms)
%OUR_ALGO 此处显示有关此函数的摘要
%   此处显示详细说明
N = size(y,1);
Y = diag(y);
[Kk,M] = kerMatForMiss(X_miss,X_miss,maskInv,'Gaussian',pms.sigma);
% lowBound = exp(-M/(2*(pms.sigma^2))); % lowBound <= K_Delta;
% upBound = ones(N,N); % K_Delta <= upBound;
